...
class Abs(NoiseModule):
    ...
    def get_values(
        ...
        return np.absolute(
            self.source0.get_values(
                width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
            )
        )
...
class Add(NoiseModule):
    ...
    def get_values(
        ...
        a = self.source0.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        b = self.source1.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
...
class Billow(NoiseModule):
    ...
    def get_values(
        ...
        if use_arrays is None:
            xa, ya, za = self.create_arrays(
                width, height, min_x, max_x, min_y, max_y, min_z, max_z
            )
        else:
        ...
        for octave in range(self.octaves):
            seed = (self.seed + octave) & 0xFFFFFFFF
            signal = gpu.gradient_noise(
                xa, ya, za, np.int32(seed), np.int32(self.quality.value)
            )
...
class Blend(NoiseModule):
    ...
    def get_values(
        ...
        v0 = self.source0.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        v1 = self.source1.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        alpha = (
            self.source2.get_values(
                width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
            )
            + 1
        ) / 2
...
class Displace(NoiseModule):
    ...
    def get_values(
        ...
        xD = xa + self.source0.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        yD = ya + self.source1.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        zD = za + self.source2.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )

        if use_arrays is None:
            return self.source3.get_values(
                width,
                height,
                min_x + np.amin(xD),
                max_x + np.amax(xD),
            ...
            return self.source3.get_values(
                width, height, 0, 0, 0, 0, 0, 0, [xD, yD, zD]
            )
...
class Exponent(NoiseModule):
    ...
    def get_values(
        ...
        values = self.source0.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
...
class RotatePoint(NoiseModule):
    ...
    def get_values(
        ...
        if use_arrays is None:
            return self.source0.get_values(
                width,
                height,
                min_x + np.amin(nx),
                max_x + np.amax(nx),
            ...
            return self.source0.get_values(
                width, height, 0, 0, 0, 0, 0, 0, use_arrays=[nx, ny, nz]
            )
...
class ScalePoint(NoiseModule):
    ...
    def get_values(
        ...
        if use_arrays is None:
            ...
            return self.source0.get_values(
                width, height, 0, 0, 0, 0, 0, 0, use_arrays=[xa, ya, za]
            )
...
class Select(NoiseModule):
    ...
    def __init__(
        self, source0, source1, source2, edge_falloff=0, lower_bound=-1, upper_bound=1
    ):
        ...
        if self.edge_falloff > 0:
            if control_value < (self.lower_bound - self.edge_falloff):
                ...
                return linear_interp(
                    self.source0.get_value(x, y, z),
                    self.source1.get_value(x, y, z),
                    alpha,
                )
    ...
    def get_values(
        ...
        s0a = self.source0.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        s1a = self.source1.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        s2a = self.source2.get_values(
            width, height, min_x, max_x, min_y, max_y, min_z, max_z, use_arrays
        )
        ...
        for i in range(self.octaves):
            ...
            if t0 < 0:
                ...
                n0 = (
                    ...
                    * t0
                    * self.grad(
                        self.perm[ii + self.perm[jj + self.perm[kk]]], x0, y0, z0
                    )
                )
            ...
            if t1 < 0:
                ...
                n1 = (
                    ...
                    * t1
                    * self.grad(
                        self.perm[ii + i1 + self.perm[jj + j1 + self.perm[kk + k1]]],
                        x1,
                        y1,
                        z1,
                    )
            ...
            if t2 < 0:
                ...
                n2 = (
                    ...
                    * t2
                    * self.grad(
                        self.perm[ii + i2 + self.perm[jj + j2 + self.perm[kk + k2]]],
                        x2,
                        y2,
                        z2,
                    )
            ...
            if t3 < 0:
                ...
                n3 = (
                    ...
                    * t3
                    * self.grad(
                        self.perm[ii + 1 + self.perm[jj + 1 + self.perm[kk + 1]]],
                        x3,
                        y3,
                        z3,
                    )
...
class TranslatePoint(NoiseModule):
    ...
    def get_values(
        ...
        if use_arrays is None:
            ...
            return self.soruce0.get_values(
                width, height, 0, 0, 0, 0, 0, 0, use_arrays=[xa, ya, za]
            )
...
class Turbulence(NoiseModule):
    ...
    def get_values(
        ...
        if use_arrays is None:
            ...
            return self.source0.get_values(
                width, height, 0, 0, 0, 0, 0, 0, use_arrays=[a, b, c]
            )
...
class Voronoi(NoiseModule):
    ...
    def get_value(self, x, y, z):
        ...
        return value + (
            self.displacement
            * value_noise_3d(math.floor(xCan), math.floor(yCan), math.floor(zCan), 0)
        )
