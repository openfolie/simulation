...
def noise_map_plane_gpu(
    ...
    if not seamless:
        return source.get_values(
            width, height, lower_x, upper_x, 0, 0, lower_z, upper_z
        )
    else:
        x_extent = upper_x - lower_x
        z_extent = upper_z - lower_z

        se = source.get_values(
            width, height, lower_x + x_extent, upper_x + x_extent, lower_z, upper_z, 0
        )
        sw = source.get_values(width, height, lower_x, upper_x, lower_z, upper_z, 0)
        nw = source.get_values(
            width, height, lower_x, upper_x, lower_z + z_extent, upper_z + z_extent, 0
        )
        ne = source.get_values(
            width,
            height,
            lower_x + x_extent,
            upper_x + x_extent,
